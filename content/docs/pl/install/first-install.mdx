...
sourceSha256: 0fbc80cda4eea3791d161326cfbdecba2ded763140127017c3bb6f96bfc0ed5a
---
...
**Trzeciej nie ma i nie ma też nic do założenia na dysku.** Do 2026-09-16 był
nią `RUNNER_WORK_DIR`; dziś Runnery trzymają wszystko, co pobierają,
przygotowują i nad czym pracują, w wolumenach Dockera. Demon zakłada je sam,
z właścicielem root i prawami 0755, a dokładnie tego trzeba: Runner zapisuje
jako root, a każdy kontener zadania montuje wolumen tylko do odczytu i czyta go
jako uid **65534**.

Jeśli Docker na tym hoście jest starszy niż **Engine 26** (albo Podman 5),
zajrzyj przed pierwszym uruchomieniem do
[Gdzie Runnery trzymają swoje bajty](/pl/install/configuration#where-the-runners-keep-their-bytes):
taka instalacja zostaje przy katalogach na hoście, a razem z nimi wraca
`RUNNER_WORK_DIR`.
...
cgroup v1 albo nieznany sterownik cgroup, `.env` śledzony przez Git.
