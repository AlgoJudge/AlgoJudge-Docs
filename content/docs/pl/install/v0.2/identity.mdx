...
source: en/install/v0.2/identity.mdx
sourceSha256: 30fd394ab4d90e1487bbda2caa6a399c438f7bcf474f2762ed7a921ad0050d0b
---
...
w katalogu — referencyjny stos Moodle zależy od tego widoku i dlatego
...
się w katalogu udostępnianym przez Twoje wdrożenie tożsamości, nie
...
samej wagi co [podłączenie platformy kursowej](/pl/install/v0.2/lti) i stoi za tym
samym uprawnieniem.
...
[Konfiguracja](/pl/install/v0.2/configuration).
...
1. **Zarejestruj dostawcę** i skopiuj **adres powrotny**, który pokazuje ekran,
   ...
   nie wymyślasz jej.
2. **Napisz mapowanie** — które grupy katalogu stają się którymi rolami. Jedna
   grupa może być warta więcej niż jedną. Zacznij od `deny`, które nie wpuszcza
   nikogo, kogo mapowanie nie nazywa.
3. **Jeśli dostawca umie zgłaszać usunięcia**, ustaw sekret kanału zwrotnego
   i włącz kanał.
4. **Jeśli nikt tutaj nie loguje się inaczej**, wskaż go ekranowi logowania,
   w [Instancji](/pl/client/manager/instance). Bez tego ekran jest stroną
   z jednym przyciskiem; z tym przeglądarka idzie prosto do dostawcy.
   `?admin=true` nadal prowadzi do formularza lokalnego, a odrzucone logowanie
   nadal ląduje na ekranie z kodem z listy niżej, zamiast iść w kółko.
...
| `unmappedBehavior` | `deny` *(domyślnie)* albo `defaultRole` |
...
Reguła mapowania to **wartość** claimu i **role**, które ona nadaje. Wartość
porównuje się jako napis, znak w znak — nie ma wzorców, bo znak wieloznaczny na
liście dozwolonych to sposób, w jaki lista dozwolonych przestaje nią być. Role
wybiera się z listy i reguła trzyma ich identyfikatory, więc **zmiana nazwy roli
niczego nie psuje**; psuje ją zmiana nazwy *grupy u dostawcy* — i to tej zmiany
trzeba pilnować. Dwie zasady są egzekwowane i nie da się ich wyłączyć: **`system:administrator` jest
...
poświadczenie. Patrz [Kopie zapasowe](/pl/install/v0.2/backup), gdzie to samo dotyczy
pęku kluczy.
</Callout>
...
- **Dostawca nigdy nie przejmuje istniejącego konta po nazwie ani po adresie.**
  Kolidująca nazwa użytkownika dostaje krótki skrót wartości `sub`, a kolidujący
  adres pomijamy i konto powstaje bez niego.
- **Konto może mieć kilka powiązań, a logowanie samo z siebie nie łączy dwóch.**
  Login uczelniany i `auth.algojudge.app` to dwie drogi do tej samej osoby i jedno
  konto trzyma oba powiązania. Żadne logowanie nie rozstrzyga natomiast
  samodzielnie, że nowy `sub` należy do konta, które już istnieje — jedynym
  dowodem, jakim by dysponowało, jest adres, którego nikt nie zweryfikował, a
  automatyczne kojarzenie po takim adresie to przejęcie konta, a nie ułatwienie.
  Konta łączy się uprawnieniem `user:merge`: to czynność administracyjna, którą
  ktoś podejmuje świadomie i którą da się cofnąć, dopóki opróżnione konto jest
  jeszcze całe.
