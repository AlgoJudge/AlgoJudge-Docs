...
source: en/install/v0.2/reverse-proxy.mdx
sourceSha256: 35423552fd8518f73a19e82b3800e1c12c0037de7a0aeb60525b79b6a8bea5d0
---
...
własnym proxy ta domyślna jest zła** — wpisz tam sieć swojego proxy.
**Opróżnienie tej linii opróżnia listę**: wartość domyślna zastępuje linię,
której nie ma, a nie linię pustą.
...
**`none` należy do `TRUSTED_PROXY_PROXIES`, czyli do linii poniżej.** Jeśli
przed Serverem nie stoi nic, wpisz `TRUSTED_PROXY_PROXIES=none` i zostaw
`TRUSTED_PROXY_NETWORKS` puste — Server przestanie wtedy w ogóle czytać
`X-Forwarded-For`. W linii sieci to samo słowo jest czytane jako blok CIDR
i odrzucane przy starcie, a `none` obok sieci, która nadal jest wymieniona,
zostaje odrzucone tak samo: Server przyjmuje je wyłącznie jako całość tego,
czemu kazano mu ufać. Obie linie puste też są odrzucane, z powodu podanego na
początku tej reguły. `preflight.sh` zgłasza każdy z tych trzech przypadków przed
startem.
...
## Co sprawia, że Client daje się zainstalować

Client instaluje się jak aplikacja: przeglądarka, która dostanie manifest,
zapamiętuje ikonę, otwiera aplikację we własnym oknie i rysuje ją z kopii
zapisanej na urządzeniu, gdy sieci nie ma. Co z tego ma uczestnik, opisuje
[jego własna strona](/pl/client/participant). O tym, czy dostanie to w ogóle,
decydują trzy nagłówki odpowiedzi, a **obraz Clienta wysyła wszystkie trzy** —
więc za proxy, które przepuszcza kontener, nie ma nic do dodania. Nie buforuj
przed nim i nie podmieniaj jego `Cache-Control`.

**TLS nie jest tu opcjonalny.** Service worker rejestruje się wyłącznie w
bezpiecznym kontekście, więc instalacja wystawiona po zwykłym `http://` to
instalacja bez kopii na urządzeniu i bez propozycji instalacji. Nic tego nie
zgłosi: aplikacja działa, tylko nigdy nie da się jej zainstalować.

Jeżeli zbudowane pliki serwujesz sam, zamiast kierować ruch na kontener, tak to
wygląda w wersji dostarczanej.
...
`@app` to miejsce, w które trafia `try_files $uri $uri/ @app`, więc `no-store`
obejmuje **każdą nawigację**, a nie tylko dosłowne `GET /index.html`. To
nawigacja niesie bieżące skróty zasobów i adres API, który kontener wpisał przy
starcie, więc ta z bufora podaje przeglądarce poprzednie wydanie i nieaktualny
adres.

`types { }` opróżnia mapę typów w tej jednej lokalizacji, żeby decydował
`default_type`. To, czy dana wersja nginxa zna `.webmanifest`, bywa różne, a
manifest podany jako `application/octet-stream` przeglądarka pomija — po cichu,
przy działającej aplikacji, która przestała być instalowalna.

<Callout type="warn" title="Zbuforowany `/sw.js` przeżyje poprawkę">
Worker jest tym jedynym plikiem, który decyduje o buforowaniu całej reszty, więc
przeglądarka z jego kopią trzyma się starych reguł tak długo, jak długo tę kopię
ma, a do niej nie sięgniesz. `no-cache` na tej jednej ścieżce jest tym, co
pozwala pomyłkę naprawić.
</Callout>
...
<Callout type="warn" title="Musi to być ta sama witryna: jedna domena rejestrowalna, jeden schemat">
...
Każdy układ przetrwa to, co nie potrzebuje sesji w ogóle: znak firmowy
instalacji, krój pisma i dokumenty publikowane przez operatora są plikami
publicznymi i wczytają się skądkolwiek.
...
<Callout type="warn" title="Odmowa jest cicha">
...
Jeden indeks na origin — `AJ_Cors__AllowedOrigins=a,b` nie wiąże **żadnego**, po
...
jest strona przerwy Clienta z napisem *Server nie odpowiada*, a nie brakujący
obrazek.
