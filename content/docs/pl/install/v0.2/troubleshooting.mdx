...
source: en/install/v0.2/troubleshooting.mdx
sourceSha256: fb2a3a1413da1014e782ed06700e8769d206d7913a6926108bde39db1789f9b6
---
...
3. **Demon jest starszy niż Docker Engine 26.** Paczka jest rozpakowywana we
   ...
   tym, zamiast oceniać na niczym. `./scripts/preflight.sh` odmawia wcześniej
   niż pierwsze zgłoszenie, a
   `docker version --format '{{.Server.APIVersion}}'` podaje tę liczbę.
   Zaktualizuj demona; poniżej tego progu nie ma rozwiązania zastępczego.
4. **Wolumen pamięci podręcznej nie jest tym, o którym powiedziano Runnerowi.**
   Każdy Runner dostaje nazwę wolumenu, o który ma poprosić demona, i kończy
   pracę przy starcie, gdy demon takiego wolumenu nie ma — jest to więc Runner,
   który nigdy się nie rejestruje, a nie taki, który źle ocenia. Dwa polecenia
   z wpisu *Checker zawodzi na każdym zgłoszeniu* poniżej wypisują to, co Runner
   dostał, i to, czym dysponuje demon.
...
Dwie przyczyny, od najczęstszej:
...
2. **Katalog roboczy nie potrafi trzymać nazwanego potoku.** Kanał to FIFO
...
Runner, który w ogóle nie widzi swojej pamięci podręcznej, odmawia **startu** i
...
[Zewnętrzny sędzia](/pl/install/v0.2/external-runner) i obie zostawiają pustą kolejkę
...
wycofanie](/pl/install/v0.2/update).
...
**Druga: kontener zadania nie potrafi odczytać przestrzeni roboczej** — co przy
wolumenach jest mało prawdopodobne: demon zakłada wolumen z właścicielem root
i prawami 0755, a to jest już poprawne. Runner zapisuje tam pliki zgłoszenia
jako root, a każdy kontener zadania montuje ten katalog tylko do odczytu i czyta
go jako uid **65534**. Wolumen podmieniony ręcznie potrafi nie spełnić tej
drugiej połowy, więc gniazdo działa bez zarzutu, Runner rejestruje się, bierze
