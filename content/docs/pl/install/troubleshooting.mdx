...
sourceSha256: e25df352c80941881c817636c2c01c38b416e997f431b6caf7f202486a70e6b6
---
...
**`RUNNER_CACHE_DIR` nie jest katalogiem, który demon Dockera potrafi otworzyć.**
Paczka jest rozpakowywana, a jej checker budowany raz, w pamięci podręcznej, i
kontener sędziego montuje jedno i drugie — a montowanie rozwiązuje demon, więc
katalog musi istnieć na hoście i być czytelny dla uid **65534**, dokładnie tak
jak katalog roboczy.
...
Runner wskazany na ścieżkę, której demon w ogóle nie potrafi otworzyć, odmawia
oceniania i mówi o `AJ_Cache__HostPath`. Ten wpis dotyczy przypadku węższego:
katalog istnieje, ale nie jest tym, do którego Runner zapisuje.
...
**Druga: kontener zadania nie potrafi odczytać `RUNNER_WORK_DIR`.** Runner
zapisuje tam pliki zgłoszenia jako root, a każdy kontener zadania montuje ten
katalog tylko do odczytu i czyta go jako uid **65534**. Katalog ręcznie zamknięty
...
wszystkie przebiegi, więc szczyt bierze się z wyzerowania `memory.peak` —
interfejsu jądra, który pojawił się w **Linuksie 6.12**, a więc już nie
w Ubuntu 24.04, za to w Debianie 13. O limicie czasu rozstrzyga czas procesora,
więc każdy werdykt pozostaje w mocy, a szczyt samego ocenianego zgłoszenia też
nie cierpi: ten czyta się z cgroup założonej świeżo na ten przebieg. Brakuje
szczytu przebiegów, które nie są niczyim zgłoszeniem — kompilacji, programu
weryfikującego, interaktora.
