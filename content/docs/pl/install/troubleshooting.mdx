...
sourceSha256: b86f27a046f0d4660c661cdc6a5ee9b0e4a841acc124d22ee141cd1d107af2ba
---
...
**Drugą przyczynę zamknięcia tej powierzchni widać w innym komunikacie.** Server
wypisuje go w logu przy starcie:

```text
AJ_Admin__Token is the well-known development token, so /admin is closed.
```

To instalacja z wartością, którą dostarcza stos deweloperski,
...
trafia do wszystkich czterech usług z tej samej kotwicy, więc jeden wąski
`RUNNER_2_CPUSET` to pętla na `runner-2`, podczas gdy `runner-1` ocenia bez
zarzutu — czytaj log tego, który się zapętlił, a nie tego, który działa.

**Runner bez cpusetu czyta całą maszynę.** Zostawienie pustego zbioru nie
rozluźnia sprawdzenia: szerokość cztery na maszynie o dwóch procesorach zostanie
odrzucona, czy zbiór jest wąski, czy go nie ma.
