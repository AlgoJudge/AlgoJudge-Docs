...
sourceSha256: 0d69b99fa0c5bec3da66d056c2f2bd85ac32634a9d8b3284397efc8c9c21fa50
---
...
| `RUNNER_1_CPUSET` … `RUNNER_4_CPUSET` | puste | z których procesorów może korzystać każdy Runner. Puste zostawia rozmieszczenie planiście hosta. Podział, który warto zrobić, to **jeden Runner na grupę procesorów i jeden tor na procesor w tej grupie** — ile torów, mówi `RUNNER_TESTS_AT_ONCE` niżej, a tor na cały *rdzeń*, nie na wątek, to podział czysty, którego Runner za ciebie nie sprawdzi. Runner tnie zbiór na tory w takiej kolejności, w jakiej go zapisano, więc odczytaj ją z maszyny, zamiast przepisywać przykład: `thread_siblings_list` mówi `0-1` na jednym hoście, a `0,8` na innym. `RUNNER_3_CPUSET` i `RUNNER_4_CPUSET` należą do tych dwóch Runnerów, które uruchamia `runner-extra`. Patrz [Czego wymaga host](/pl/install/requirements) |
