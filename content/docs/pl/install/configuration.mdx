...
sourceSha256: 232022edda015eefa71974bddc122ad606c8ecd7f20c01f0235876ef2988f90b
---
...
| `APP_BASE_URL` | puste | jego odbicie lustrzane: adres **aplikacji** tak, jak sięga do niej przeglądarka. Puste jest właściwe przy jednym origin. Przy dwóch pozostawienie go pustym kończy logowanie federacyjne na gołym `/activities`, rozwiązanym względem origin API |
...
| `RUNNER_STOP_GRACE` | `300s` | ile Docker czeka, zanim zabije Runnera. **Nic nie daje**: Runner nie obsługuje `SIGTERM`, więc w tym czasie niczego nie kończy — a `docker compose down` odczekuje to do końca. Czyste zatrzymanie to najpierw drenowanie |
...
| `EXTERNAL_PROBLEM_TYPES` | puste | puste deklaruje własny typ archiwum. Literówka jest tu ciszą, a nie błędem |
...
przy starcie, domyślne wartości obrazu spełniają to wszystko, a zmiana w dobrej
wierze — łagodniejsze odpytywanie cudzej usługi — jest właśnie tą, która to
psuje.
