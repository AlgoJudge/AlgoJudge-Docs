...
| `RUNNER_STOP_GRACE` | `300s` | ile Docker czeka, zanim zabije Runnera. **Nic nie daje**: Runner nie obsługuje `SIGTERM`, więc w tym czasie niczego nie kończy — a `docker compose down` odczekuje ten czas do końca. Czyste zatrzymanie zaczyna się od drenowania |
