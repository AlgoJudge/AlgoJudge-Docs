...
| `RUNNER_CACHE_DIR` | `/srv/algojudge/runner-cache` | an **absolute host path** the four Runners share. Each package is downloaded, unpacked and its checker built there once between them, under a lock; each Runner bounds it at 10 GiB from inside. A directory rather than a named volume because a checker's container mounts the unpacked package out of it, and the daemon resolves that by path. **Keep it out of `RUNNER_WORK_DIR`**, whose contents the scheduled clean-up removes by age |
...
| `RUNNER_1_CPUSET` … `RUNNER_4_CPUSET` | empty | which processors each Runner may use. Empty lets the host's scheduler place every job. The division worth making is one Runner per physical core, given both of that core's threads — see [What the host needs](/en/install/requirements) |
