...
| `RUNNER_STOP_GRACE` | `300s` | how long Docker waits before killing a Runner. It **buys nothing**: the Runner handles no `SIGTERM`, so it does not finish anything in that time — and `docker compose down` waits the whole of it. A clean stop is a drain first |
