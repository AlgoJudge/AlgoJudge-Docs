...
scratch became Docker volumes. There is no host path to choose anymore.
</Callout>
...
Its lease and its polling intervals are deliberately not offered: the image's
defaults are sound and a few of the values still check one another at startup.
