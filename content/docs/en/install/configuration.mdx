...
| `RUNNER_CACHE_DIR` | `/srv/algojudge/runner-cache` | an **absolute host path** every Runner on the host shares. Each package is downloaded, unpacked and its checker built there once between them, under a lock; each Runner bounds it at 10 GiB from inside. A directory rather than a named volume because a checker's container mounts the unpacked package out of it, and the daemon resolves that by path. **Keep it out of `RUNNER_WORK_DIR`**, whose contents the scheduled clean-up removes by age |
