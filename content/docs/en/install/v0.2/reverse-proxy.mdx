...
`/api/v1` — is normalized rather than refused.
...
own proxy, that default is wrong** — put your proxy's network there. **Emptying
the line empties the list**: the default stands in for a line that is absent, not
for a blank one.

Write a **network** address, not a host one: `172.28.0.5/24` is refused at
startup, by name, with the address it should have been. .NET normalizes a CIDR
with host bits set without a word, which would turn "one machine" into "a whole
laboratory".

**`none` belongs to `TRUSTED_PROXY_PROXIES`, the line under it.** For a Server
nothing sits in front of, write `TRUSTED_PROXY_PROXIES=none` and leave
`TRUSTED_PROXY_NETWORKS` empty; the Server then stops reading `X-Forwarded-For`
altogether. In the networks line the same word is read as a CIDR block and
refused at startup, and `none` beside a network that is still named is refused
too — the Server takes it only as the whole of what it has been told to trust.
Both lines empty is refused as well, for the reason this rule opens with.
`preflight.sh` reports each of the three before a start.
...
## What makes the Client installable

The Client installs: a browser that is given the manifest keeps an icon, opens
the application in a window of its own, and draws it from a copy on the device
when the network is gone. What a participant gets out of that is
[their own page](/en/client/participant). Three response headers decide whether
they get it at all, and **the Client image already sends all three** — so behind
a proxy that passes the container through there is nothing to add. Do not cache
in front of it, and do not replace its `Cache-Control`.

**TLS is not optional for this.** A service worker registers only in a secure
context, so an installation reached over plain `http://` is one with no copy on
the device and no install prompt. Nothing reports it: the application works, it
simply never becomes installable.

Where you serve the built files yourself rather than proxying the container,
this is the shipped form.
...
`@app` is where `try_files $uri $uri/ @app` lands, so `no-store` covers **every
navigation** rather than only a literal `GET /index.html`. A navigation is what
carries the current asset hashes and the API address the container wrote in at
start-up, so a cached one hands a browser the previous build and a stale
address.

`types { }` empties the type map for that one location so `default_type`
decides. Whether an nginx build knows `.webmanifest` varies by version, and a
manifest served as `application/octet-stream` is one the browser ignores —
silently, with the application still working and no longer installable.

<Callout type="warn" title="A cached `/sw.js` outlives the fix">
The worker is the one file that decides how everything else is cached, so a
browser holding a copy holds the old rules for as long as it keeps it, and you
cannot reach into that browser. `no-cache` on this single path is what keeps a
mistake correctable.
</Callout>
...
being refused; the symptom of a missing origin is the Client's maintenance page
saying *The Server is not answering*, not a missing picture.
