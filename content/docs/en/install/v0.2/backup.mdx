...
a certificate nobody supplies anymore fails exactly there. Without asking, the
...
backup first. [Where the files go](/en/install/v0.2/storage) is the whole of it.
...
- **`pg_restore --list`** runs on every dump. A custom-format archive keeps its
  table of contents at the front, so this succeeds on a file truncated halfway
