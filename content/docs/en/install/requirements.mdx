...
**A Runner also holds one test file in memory while it judges it** — the input
it hands the submission, which is a copy rather than a mount — so a host judging
problems with very large tests wants room for the largest of them per Runner, on
