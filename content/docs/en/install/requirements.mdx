...
cannot do is carry the arrangement a memory limit is enforced with — a cgroup
per run holding the submission alone — or measure what a process that has
already exited used.
...
| A **judged** run's peak memory | `memory.peak` of the cgroup the submission was in, made per run | the same, and made per run inside the slice |
| Any other run's peak memory | read from `memory.peak` | `memory.peak` reset at the start of the run and measured from what the slice already held, which needs **Linux 6.8** |
...
enforced on a cgroup holding the submission alone, and that one is the Runner's
to make whichever backend is in use — so a tree it cannot write into is a
**refusal to start** either way, checked at startup rather than found out one
