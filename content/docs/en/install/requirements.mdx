...
description: Docker with Compose v2.15.0 and, where Runners judge, Engine 26; cgroup v2, the coreutils the scripts use, and disk for backups and volumes.
---
...
```bash
docker version --format '{{.Server.APIVersion}}'    # 1.45 or higher
```

**An installation below that floor is still supported.** It keeps the host
directories this stack used until 2026-09-16 — see
[Where the Runners keep their bytes](/en/install/configuration#where-the-runners-keep-their-bytes).
Nothing else in the stack needs 26, so an application host with no Runners is
unaffected.
...
| Physical cores | Lanes |
|---|---|
| 4 | 2 |
| 8 | 4 |
| 16 | 8 |

**Lanes rather than Runners, because the two are not the same count.** The
`runner` profile starts two Runners and `runner-extra` the other two, so the
middle row is two Runners of two lanes as readily as four of one. Choosing
between those is a question of how many submissions are judged at once against
how soon each of them is answered; it is not a question of size.
...
lanes, lower `RUNNER_TESTS_AT_ONCE` or leave `runner-extra` out of
`COMPOSE_PROFILES`.
...
The 0.6 GiB a lane is the Runner's own process and the largest container that
lane will hold. **A lane holds one judged container at the problem's memory limit
plus 64 MiB, and a checker or interactor beside it at 256 MiB**, so four lanes on
256 MiB problems is about 1.3 GiB for that Runner before inputs. The two builds
stand outside the lanes — one at a time, capped at 512 MiB — and 0.6 GiB a lane
is the figure that covers both shapes.

### A worked example: four lanes

One Client, one Server, one External Runner, and four judging lanes — two
Runners of two, or four of one:
...
lane**, and one core per lane on top of two for everything else.
...
**The division worth making is one Runner per group of processors, and one lane
per processor in the group.** Two *different* Runners on one core take each
...
**Two lanes on the two threads of one core do exactly that, and this is the half
nothing checks.** A Runner refuses a width its set cannot give one processor
each; whether those processors are separate cores is yours to arrange. A lane to
...
cpu0's partner is cpu8. `AlgoJudge-Ops` exposes the two halves as `RUNNER_1_CPUSET` and
its siblings and as `RUNNER_TESTS_AT_ONCE`, and a Runner given more lanes than
...
below. Beyond that the Runners share a package cache bounded at 10 GiB and the
External Runner one bounded at 256 MiB. **That 10 GiB holds more than the
downloads**: a package sits there with the tests unpacked out of it and the
checker it declares already compiled, prepared once rather than per submission,
so the same ceiling holds fewer packages than it would have held archives.

**A Runner also holds one test file in memory per lane while it judges it** — the
input it hands the submission, which is a sealed copy rather than a mount — so a
host judging problems with very large tests wants room for the largest of them
per lane, on top of the figures above.
...
On a machine whose `/var` is small and whose `/srv` is not, either move
Docker's data root or keep host directories, as above.
