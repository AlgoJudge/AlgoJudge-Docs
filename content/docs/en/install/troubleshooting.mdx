...
3. **The daemon is older than Docker Engine 26.** A package is unpacked into
   ...
   reports it before a submission does, and
   `docker version --format '{{.Server.APIVersion}}'` is the number. Either
   update the daemon, or keep host directories — see
   [Where the Runners keep their bytes](/en/install/configuration#where-the-runners-keep-their-bytes).
4. **`RUNNER_WORK_DIR` is wrong**, which is a cause only where that overlay is
   in use. This is the nastiest one, because it fails *silently*: the Docker
   daemon is handed this path directly, and a path it cannot open produces an
   **empty directory** rather than an error. Every job then runs against
   nothing. It must be **absolute**, and it must be a path the daemon — not
   only your shell — can open.
...
2. **The work directory is not the one the daemon sees**, which is a cause only
   where host directories are in use. A run's channels are made in the job's
   scratch and bind-mounted into the container, and a path the daemon cannot
   open produces an **empty directory** rather than an error — so there is no
   pipe inside for the shim to open. `RUNNER_WORK_DIR` must be absolute and
   openable by the daemon, not only by your shell.
3. **The scratch cannot hold a named pipe.** A channel is a FIFO and a Unix
...
By default the cache is the volume `algojudge_runner-cache`, reached by name,
which leaves nothing to get wrong; this entry is then almost always a Runner
pointed somewhere else by hand:
...
**Where host directories are in use** it is a path, and a path the daemon
cannot open is the ordinary cause. It has to exist on the host and be readable
by uid **65534**, exactly as the work directory is.
...
A Runner that cannot see its cache at all refuses to start judging and says so,
naming `AJ_Cache__HostPath` or `AJ_Cache__Volume`, so this entry is the
narrower case: a cache that exists and is not the one the Runner writes to.
...
**The width is one value for every Runner.** `RUNNER_TESTS_AT_ONCE` reaches all
four services from the same anchor, so one narrow `RUNNER_2_CPUSET` is a loop on
`runner-2` while `runner-1` judges perfectly — read the log of the one that is
looping, not of the one that works.

**A Runner with no cpuset reads the whole machine.** Leaving the set empty does
not make the check lenient: a width of four on a machine with two processors is
refused whether the set is narrow or absent.
...
**Two: a job container cannot read `RUNNER_WORK_DIR`**, which is a cause only
where host directories are in use — the daemon creates a volume root-owned and
mode 0755, and that is already right. The Runner writes a submission's files
there as root, and every job container mounts the directory read-only and reads
it as uid **65534**. A directory locked down by hand passes for the Runner and
fails for the job, so the socket works perfectly, the Runner registers, claims
a job — and every single one fails at once, which is how to tell the two
apart.
...
each package there as root and a judge's container reads it back as 65534. `sudo
chmod 755 /srv/algojudge/runner-cache`, and `scripts/preflight.sh` probes both.
