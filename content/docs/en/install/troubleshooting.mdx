...
A Runner probes each image once, the first time it uses it, and remembers the
answer for as long as the process lives. One that has already decided an image
has no shim goes on failing every job against the corrected copy until it is
recreated.
</Callout>
