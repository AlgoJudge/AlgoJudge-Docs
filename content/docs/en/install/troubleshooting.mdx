...
`preflight.sh` runs exactly this and refuses if they disagree — asked of a
container rather than of your shell, because under Docker Desktop and WSL the
socket this shell can see is not the one the daemon serves. When this is the
cause, the Runner fails at **startup** and never reaches a job.
