...
**`RUNNER_CACHE_DIR` is not a directory the Docker daemon can open.** A package
is unpacked and its checker built once, in the cache, and a judge's container
mounts both — and the daemon resolves that bind, so the directory has to exist on
the host and be readable by uid **65534**, exactly as the work directory is.
...
A Runner pointed at a path the daemon cannot open at all refuses to start
judging and says so naming `AJ_Cache__HostPath`, so this entry is the narrower
case: a directory that exists and is not the one the Runner writes to.
