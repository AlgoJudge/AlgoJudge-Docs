...
What bounds it is not this product but whatever sits between the Runner and the
Server — an Azure Application Gateway ends a silent request at 20 s, a stock
nginx or an AWS ALB at 60 s, Cloudflare at 125 s. Twenty-five clears all but the
first. Raise it on a path you own; lower it, or set `0`, behind a proxy you do
not control.
