...
| `AJ_Cache__HostPath` | the value of `AJ_Cache__Path` | The same directory as the **container runtime's daemon** sees it |
...
**`AJ_Cache__HostPath` is why a shared cache is a directory and not a volume.**
A judge's container is given the unpacked package and the program built from it
by bind mount, and the daemon resolves a bind mount — so it has to be a path the
daemon can open. The Runner writes a marker at start and reads it back through a
throwaway container, and refuses to judge if it cannot see it.
