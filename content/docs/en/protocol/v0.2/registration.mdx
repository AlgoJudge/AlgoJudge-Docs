...
need a canonical serialization every implementation agrees on forever, and the
...
`tags` is normalized: trimmed, lowercased, de-duplicated. Absent means the default
...
`runner:*` permissions and none of them is in the shipped `manager` role, so
...
that exited because nobody had gotten to it yet would need something else watching
it.
...
both properties matter: without single use, a captured exchange is replayable
forever; without expiry, one captured today is usable next year.
