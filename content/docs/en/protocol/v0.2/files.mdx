...
the other way around — a `404` for an id nobody issued, a `403` for a job another
Runner holds — because a Runner has to be able to tell a reclaimed job from a
Server that has forgotten it. [Claiming](/en/protocol/v0.2/claiming) says why.
</Callout>
...
[infrastructure failure, not a verdict](/en/protocol/v0.2/reporting): the submission
...
That is also why [`extra` is bounded at 2 kB](/en/protocol/v0.2/reporting): the small
...
failure is the one thing that gets lost.
</Callout>
