...
`aj-admin` has no migrate command and the image carries no SDK, so the documented
alternative needs a workstation with the source — which a self-hosted stack does
not have. Before it existed, a fresh installation had every migration pending and
never started.
...
| `Cors:AllowedOrigins` | `AJ_Cors__AllowedOrigins__0`, `__1`, … | none — no CORS policy is added |
...
## Six ways the Server refuses to start
...
A pending migration is the sixth, and the only one an operator can wave through:
`Database:MigrateOnStart` is how.
