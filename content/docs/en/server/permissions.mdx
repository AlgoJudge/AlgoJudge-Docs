...
<Callout type="info" title="A provider's contribution reaches its holders later">
An identity provider's [mapping rule](/en/server/identity) names roles, and the
contribution it writes links them like any other grant. It is rewritten from
those rules at **every sign-in**, so editing a role a rule names reaches those
people at their next sign-in rather than immediately.
</Callout>
