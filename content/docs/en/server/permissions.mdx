---
title: Permissions
description: The string form, the two scopes, the resolution rules, the three shipped roles, and all 54 keys.
---
...
4. **What one grant carries is itself a union** — the role it points at, and any
   entries of its own beside it. Either half may be empty.
5. **There is no subtraction anywhere in the model.**
6. **Nobody may grant, or map onto, a permission they do not themselves hold.**
   Enforced where a grant is written — and where a role is written, since a
   grant points at one — never where it is read.

<Mermaid chart={`
flowchart TD
    q(["May this person do this, here?"])
    q --> ov{"An activity grant here<br/>carrying OverrideSystem?"}
    ov -->|yes| only["That grant alone decides.<br/>Not even system:administrator reaches in"]
    ov -->|no| adm{"system:administrator in any<br/>system contribution?"}
    adm -->|yes| bypass["Every permission check passes.<br/>Membership is asked separately"]
    adm -->|no| union["The union: every system contribution, plus<br/>the activity grant when one activity is asked about"]
    union --> each["Each of those grants is<br/>its role plus its own entries"]
    each --> held{"Is the key in that set?"}
    held -->|yes| allowed["Allowed"]
    held -->|no| refused["Refused. Nothing subtracts,<br/>and there is nothing else to consult"]
`} />

<Callout type="warn" title="There is no deny list to reach for">
...
**An activity named in a query is a question at that scope**, and the key is
required there — no key in that activity, no list. **No activity is a different
question**, *everything I may read*, and the answer to that one is a narrowing.

So a manager whose only grant is on one course opens the panel's unfiltered
submissions list and is shown that course. Asking at system scope instead
refuses them, because a grant on an activity contributes nothing to a question
asked with no activity in it — and that is every manager an activity's grant
editor makes.

**Holding the key nowhere is still a refusal.** An empty page would tell
somebody who may not look that there is nothing to see, which is a different
...
<Callout type="info" title="Some surfaces ask only *anywhere*">
...
`problem:share` and the enrolment picker's `user:read:all` ask the same way.
...
A grant **points at** a role. Editing one changes what everybody holding it may
do, at once — including people enrolled long before the edit.

That is the opposite of what this page said until 2026-09-13, when a grant held
a copy instead. What made it change was the cost being paid rather than
predicted: a permission added by a new version reached nobody who already held a
grant, and every installation had to re-issue them by hand.

<Callout type="warn" title="An edit reaches everybody at once">
This is a live link, so a role is worth treating as one. The panel shows how
many grants an edit would reach before you save it, an activity's own role
reaches only that activity, and nobody may put into a role a permission they do
not themselves hold.
</Callout>

<Callout type="info" title="One kind of grant still holds a copy">
An identity provider's [mapping rule](/en/server/identity) names a role, and the
contribution it writes is a copy — a claim may match several rules and the
contribution is the union of every role they name, which one link cannot say. It
is rewritten from those rules at **every sign-in**, so editing a role reaches
those people at their next sign-in rather than immediately.
</Callout>

A role belongs either to the installation or to **one activity**. An activity's
role may only be granted inside it, and is written by whoever holds `role:manage`
there — which is how a manager corrects what their own participants may do
without touching what every participant in the installation may do.

### `participant` — 8 keys

```
activity:read        submission:read:own    submission:create    result:read:own
question:read:own    question:create        ranking:read         printout:request
```
...
The other seventeen are deliberately **not** included: `activity:create`,
...
four `runner:` keys, `instance:update`, `provider:manage` and
`system:administrator`.
A manager runs an activity; they do not run the installation it lives in.

`role:read` and `role:manage` are in the set, and they are scoped: held in an
activity grant they reach that activity's roles and nothing else.
...
questions: *does the participant role grant it* and *does holding it make
somebody staff*. They differ for exactly one key.

<Callout type="warn" title="`trial:run` is held without becoming staff">
It is outside the participant role, because a trial spends a Runner and
...
It is staff-conferring, and that has a consequence worth knowing before you hand
it out: holding it in an activity takes the holder out of that activity's
ranking. Reading source other people wrote is what staff means, and a volunteer
who is also competing has to be given the key in an activity they do not
compete in.
...
| `role:manage` | both | create, edit and delete roles, at the scope it is held |
...
writing a role**, because a grant points at one: nobody may put into a role a
permission they do not themselves hold.

`role:manage` is scoped, and the scope is the whole of what makes it safe to
hand to a manager. Held at system scope it writes the installation's roles; held
in an activity grant it writes that activity's, and nothing else.
...
applying a role is exactly what a manager does when they enrol somebody by hand,
