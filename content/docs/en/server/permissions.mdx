...
It is a bypass **only** in a system-scoped grant, and since 2026-08-31 writing it
into an activity grant is refused outright rather than stored and ignored — the
code is `grant.permission.scope`, and the message says the key is
installation-wide and means nothing in an activity.
