---
title: Permissions
description: The string form, the two scopes, the resolution rules, the three shipped templates, and all 54 keys.
---
...
flowchart TD
    ...
    union --> held{"Is the key in that set?"}
...
The other twenty are deliberately **not** included: `activity:create`,
...
`printout:request` is in the participant template, so it confers nothing.
...
template contains them: a rule that refused every global key would refuse the
template this installation hands out.
...
admits somebody to their own library — which is what the `manager` template
