...
A grant says what a [user](/en/server/v0.1/concepts/user) may do within a scope. In
an [activity](/en/server/v0.1/concepts/activity), it **is** the membership.
...
| `GroupId` | the [group](/en/server/v0.1/concepts/group) this person competes as, or null |
| `State` | `Invited` or `Active` |
| `GrantedByUserId` | who issued it |

`Permissions` is filled in from a [template](/en/server/v0.1/permissions) and then
...
also applies to [event fan-out](/en/server/v0.1/events): somebody who stood down
