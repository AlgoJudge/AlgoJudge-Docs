...
A [grant](/en/server/v0.1/concepts/grant) holds a set of these strings and is scoped:
...
An identity provider's [mapping rule](/en/server/v0.1/identity) names a template, and
...
in [Identity](/en/server/v0.1/identity), and both are enforced rather than
documented.
...
- An archived [activity](/en/server/v0.1/concepts/activity) stays readable and
  accepts nothing new. It keeps its ranking and its history, because that is
  what participants come back for.
- An archived [problem](/en/server/v0.1/concepts/problem) disappears from the attach
