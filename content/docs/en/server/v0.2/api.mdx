...
The endpoint-by-endpoint reference is [REST reference](/en/server/v0.2/rest), which is
...
| A [Runner](/en/server/v0.2/concepts/runner) | its own bearer token, from a challenge it signed with its immutable key |
...
model](/en/server/v0.2/permissions), resolved per request and cached for that
...
model](/en/server/v0.2/permissions), and several open endpoints authorize themselves
...
| [Runner](/en/server/v0.2/concepts/runner) | 18 | a Runner is a machine with a key, not a person with a session |
| [LTI](/en/server/v0.2/lti) | 5 | a platform calls these before there is anybody to be signed in as, and each is authorized by what the platform signed |
...
[REST reference](/en/server/v0.2/rest) is built from the `openapi.json` of a pinned
