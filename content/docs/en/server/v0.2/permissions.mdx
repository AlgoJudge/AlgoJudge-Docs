---
title: Permissions
description: The string form, the two scopes, the resolution rules, the three shipped roles, and all 55 keys.
---
...
A [grant](/en/server/v0.2/concepts/grant) holds a set of these strings and is scoped:
...
4. **What one grant carries is itself a union** — every role it links, and any
   entries of its own beside them. Either half may be empty, and a grant may
   link as many roles as it is given.
...
6. **Nobody may grant, or map onto, a permission they do not themselves hold.**
   ...
   grant links one — never where it is read. It is applied to what a caller
   **adds**: taking a permission away is never refused.
...
flowchart TD
    ...
    union --> each["Each of those grants is<br/>its roles plus its own entries"]
...
A grant **links** roles — any number of them — and what it confers is their
union together with its own entries. Editing a role changes what everybody
holding it may do, at once, including people enrolled long before the edit.

Several rather than one, because the two paths that decide what somebody holds
both speak in sets: a claim may match several mapping rules, and a launch from a
course platform may carry several roles. A union is commutative, so there is no
ordering to decide and no precedence to explain.
...
<Callout type="info" title="A provider owns which roles its contribution links">
An identity provider's [mapping rule](/en/server/v0.2/identity) names roles, and the
contribution it writes links them like any other grant — so editing one of those
roles reaches its holders at once, exactly as it does anywhere else. What the
provider owns is **which** roles are linked: that set is rewritten from the rules
at every sign-in, so editing a *rule*, or moving somebody between groups at the
directory, reaches them the next time they sign in.
</Callout>
...
The other eighteen are deliberately **not** included: `activity:create`,
...
in [Identity](/en/server/v0.2/identity), and both are enforced rather than
documented.
...
- An archived [activity](/en/server/v0.2/concepts/activity) stays readable and
  accepts nothing new. It keeps its ranking and its history, because that is
  what participants come back for.
- An archived [problem](/en/server/v0.2/concepts/problem) disappears from the attach
