...
forever** — so a store id may never be reused for a different location.
...
[The admin surface](/en/server/v0.2/api).
...
`ephemeral` is refused outside Development, and an unrecognized value is refused
