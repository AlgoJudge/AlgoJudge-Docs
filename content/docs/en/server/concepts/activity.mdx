...
Since 2026-08-31 the Server refuses a limit that cannot work, on all four write
paths — the activity created, the activity edited, the assignment attached, the
assignment edited. Before that the values were stored, shown back to the manager,
and only discovered to be impossible by whoever tried to submit.
