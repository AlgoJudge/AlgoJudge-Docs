...
**Authorisation here is opt-out, not opt-in.** A fallback policy requires an
...
model](/en/server/permissions), and several open endpoints authorise themselves
...
| Signing in | 10 | the act of signing in is what starts a session, and a federated one leaves and comes back. The provider's deletion back channel is here too, authorised by the shared secret the provider was registered with |
| [Runner](/en/server/concepts/runner) | 18 | a Runner is a machine with a key, not a person with a session |
| [LTI](/en/server/lti) | 5 | a platform calls these before there is anybody to be signed in as, and each is authorised by what the platform signed |
...
The helper maps `manage/*` with its own authorisation and the rest with none.
...
front of authorisation, because there is nothing there to authorise. A `401` from
...
and held to the source by a test, so a change to it is a decision somebody took
...
**So it lags the prose pages between releases.** Endpoints added after `v0.1.0`
— the printout routes among them — are described in the pages of this section
and are not in the generated reference, because the reference follows a tag
rather than a branch. It catches up when the next release is tagged.
